define(["exports", "react", "util"], function (exports, _react, _util) {
   ...
   exports.Board = exports.default = undefined;
   ...
   var Sudoku = function (_React$Component) {
      ...
      _createClass(Sudoku, [{
         ...
         value: function displayPossibles(ctx, box) {
            ...
            ctx.textBaseline = "middle";
            ctx.fillStyle = "#736d6d";
            var translate = [{ x: -20, y: -20 }, { x: 0, y: -20 }, { x: 20, y: -20 }, { x: -20, y: 0 }, { x: 0, y: 0 }, { x: 20, y: 0 }, { x: -20, y: 20 }, { x: 0, y: 20 }, { x: 20, y: 20 }];
            for (var i = 0; i < 9; i++) {
               if (box.possibles[i]) {
               ...
               _react2.default.createElement(
                  ...
                  _react2.default.createElement(
                     ...
                     { ref: this.canvas, style: {
                           height: this.height,
                           width: this.width
                        }, onClick: this.select.bind(this), onKeyDown: this.input.bind(this), tabIndex: "1" },
         ...
         for (var i = 0; i < 9; i++) {
            ...
            for (var j = 0; j < 9; j++) {
               if ((0, _util.isNumber)(board[i][j])) {
                  _this2[i][j] = new Box(Number(board[i][j]), { row: i, columb: j }, !Number(board[i][j]));
               } else {
                  _this2[i][j] = new Box(Number(board[i][j][0]), { row: i, columb: j }, Number(board[i][j][1]));
               }
            }
         }
